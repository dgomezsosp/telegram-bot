...
exports.create = async (req, res, next) => {
  try {
    let data = await SubscriptionForm.create(req.body)
    data = data.toObject()
    data.id = data._id
...
exports.findAll = async (req, res, next) => {
  try {
    ...
    const result = await SubscriptionForm.find(whereStatement)
      .skip(offset)
      .limit(limit)
      .sort({ createdAt: -1 })
      .lean()
    ...
    const count = await SubscriptionForm.countDocuments(whereStatement)
...
exports.findOne = async (req, res, next) => {
  try {
    ...
    const data = await SubscriptionForm.findById(id).lean().exec()
...
exports.update = async (req, res, next) => {
  try {
    const id = req.params.id
    const data = await SubscriptionForm.findByIdAndUpdate(id, req.body, { new: true }).lean().exec()
...
exports.delete = async (req, res, next) => {
  try {
    const id = req.params.id
    const data = await SubscriptionForm.findByIdAndUpdate(id, { deletedAt: new Date() })
